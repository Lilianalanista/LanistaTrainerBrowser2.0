...
Ext.define('LanistaTrainer.controller.ExercisesController', {
    ...
    onShowSearchHeaderUpdate: function() {
        ...
        if (store.filters.items.length > 0)
            filter = ( (store.filters.items[1]) && (Ext.isNumber(store.filters.items[1].getValue())) && (store.filters.items[1].textOptSel) ? '<div class="filterTitle lanista-filter-muscles"><span>' + Ext.ux.LanguageManager.TranslationArray.FILTER_MUSCLES + '</span> </div> <div class="filterText">' + store.filters.items[1].textOptSel+'</div> <div class="lanista-delete-search lanista-icon">&nbsp;u</div>' : '') + ((store.filters.items[0]) && (Ext.isNumber(store.filters.items[0].getValue())) && (store.filters.items[0].textOptSel) ? '<div class="filterTitle lanista-filter-exercisetype"> <span>' + Ext.ux.LanguageManager.TranslationArray.FILTER_TYPE + ' </span> </div> <div class="filterText">'+store.filters.items[0].textOptSel+'</div> <div class="lanista-delete-search lanista-icon">&nbsp;u</div>' : '') + ((store.filters.indexOfKey('machineAux') !== -1) && (store.filters.items[2].textOptSel)  ? ' <div class="filterTitle lanista-filter-aditives"><span>' + Ext.ux.LanguageManager.TranslationArray.FILTER_ADDITIVES + ' </div> </span> <div class="filterText">' + store.filters.items[2].textOptSel+'</div> <div class="lanista-delete-search lanista-icon">&nbsp;u</div>' : '');
            ...
            if (searchList.length > 0){
                ...
                if (!Ext.ComponentManager.get('recommendatiosContextMenu')){
                    contextMenu = Ext.create('Ext.menu.Menu', {
                        ...
                        shadow: true,
                        width: 100,
                        height: (searchList.length * 50) < 200 ? searchList.length * 50 : 200,
                        autoscroll: true,
