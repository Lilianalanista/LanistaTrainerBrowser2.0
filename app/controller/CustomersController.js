...
Ext.define('LanistaTrainer.controller.CustomersController', {
    ...
    searchCustomerByText: function(value) {
        var controller = this,
            ...
            customerStoreFilter = Ext.getStore('CustomerStore'),
        ...
        var filterFunction = new Ext.util.Filter({
                filterFn: function(item){
                    nameValue = item.data.first_name;
                    resultFirstName = nameValue.match(re);

                    nameValue = item.data.last_name;
                    resultLastName = nameValue.match(re);

                    nameValue = item.data.email;
                    resultMail = nameValue.match(re);

                    if ( (resultFirstName && resultFirstName.length > 0) ||
                         (resultLastName && resultLastName.length > 0) ||
                         (resultMail && resultMail.length > 0)) {
                                if (searchList.indexOf(item.data.first_name + ' ' + item.data.last_name + ' ' +item.data.email) == -1)
                                    searchList.push(item.data.first_name + ' ' + item.data.last_name + ' ' +item.data.email);
                    }
                    return ((resultFirstName && resultFirstName.length > 0) ||
                            (resultLastName && resultLastName.length > 0) ||
                            (resultMail && resultMail.length > 0));
                }
            });
        ...
        if (textToSearch && textToSearch.length) {
            customerStoreFilter.clearFilter();
            customerStoreFilter.removeFilter('filterByWord');
            customerStoreFilter.filters.add (filterFunction);
            customerStoreFilter.load({
                    ...
                    if (searchList.length > 0){
                        ...
                        if (!Ext.ComponentManager.get('recommendatiosContextMenu')){
                            ...
                            contextMenu.on('click', function(menu, item, e, eOpts) {
                                if (item){
                                    customerStore.removeFilter('filterByWord');
                                    re = new RegExp("\\w*"+item.text+"\\w*", "gi");
                                    filterFunction = new Ext.util.Filter({
                                        id: 'filterByWord',
                                        filterFn: function(itemValue){
                                            nameValue = itemValue.data.first_name;
                                            resultFirstName = nameValue.match(re);

                                            nameValue = itemValue.data.last_name;
                                            resultLastName = nameValue.match(re);

                                            nameValue = itemValue.data.email;
                                            resultMail = nameValue.match(re);
                                    ...
                                    customerStore.filters.add(filterFunction);
